...
{
SEXP myAux = VECTOR_ELT(theSEXP, theNum) ;
        for (uint i = 0 ; i < theLigne ; i++)
                for (uint j = 0 ; j < theCol ; j++)
...
{
SEXP myAux = VECTOR_ELT(theSEXP, theNum) ;
        for (uint i = 0 ; i < theMat.mNRow ; i++)
                for (uint j = 0 ; j < theMat.mNCol ; j++)
...
{       mvNbProtect++ ;
        ...
        for (uint i = 0 ; i < theVect.mSize ; i++)
                REAL(theSEXP)[i] = theVect[i] ;
}
