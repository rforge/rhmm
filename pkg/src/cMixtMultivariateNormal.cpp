...
#include "cMixtMultivariateNormal.hpp"
...
double mys = 0.0 ;
	...
	register uint l ;
		for (l = 0 ; l < mvNMixt ; l++)
		{	for (register uint k = 0 ; k < mvDimObs ; k++)
			{	mMean[i][l][k] =  -2*myStd[k] + myMoy[k] + 2*myStd[k] * gsl_rng_uniform(myr) ;
				mCov[i][l][k][k] = 0.5*myVar[k] + 3*myVar[k] * gsl_rng_uniform(myr) ;	 
			}
			mp[i][l] = gsl_rng_uniform(myr) ;	
			mySomme += mp[i][l] ;
		}
		for (l = 0 ; l < mvNMixt ; l++)
			mp[i][l] /= mySomme ;
