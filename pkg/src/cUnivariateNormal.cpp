...
#endif //_RDLL_
	// Initialisation différente de KMeans
#ifdef _GSL_NO_R_
	gsl_rng	 *myr = mr	;
#else
	int	myr = 0 ;
...
double	mys = 0.0L		;		
		...
		mystdev = sqrt(myVar) ;
		for (register uint i = 0 ; i < theInParam.mNClass ; i++)
		{	mMean[i] =  -2*mystdev + myMoy + 2*mystdev * gsl_rng_uniform(myr) ;
			mVar[i] = 0.5*myVar + 3*myVar * gsl_rng_uniform(myr) ;	 ;
		}
